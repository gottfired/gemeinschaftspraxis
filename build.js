 ...
 *   - Content:    therapists.js (practice, sections, therapists)
 ...
 *
 * Workflow after editing therapists.js or a template:
 *
 *     npm run build
...
// A JS module (not JSON) so multiline template literals are allowed in text fields.
const data = require(path.join(ROOT, 'therapists.js'));
...
/* ----------------------------------------------------------------------------
 * esc(value) – HTML-escaping for all content coming from therapists.js.
 ...
 * therapists that have a url in therapists.js (separated by " · ").
 ...
 * referenced in therapists.js into build/ (preserving folder structure).
...
function copyStatic() {
  ...
  for (const asset of referencedAssets()) {
    ...
    if (!fs.existsSync(src)) {
      throw new Error('Asset referenced in therapists.js is missing: ' + asset);
    }
...
function buildPage(templatePath, outPath) {
  ...
  // the data file could contain "$" patterns that replace would interpret as
  // regex special characters – split/join always replaces literally.
  ...
  const header =
    '<!-- This page is generated by build.js from templates/ and therapists.js. Do not edit by hand. -->\n';
